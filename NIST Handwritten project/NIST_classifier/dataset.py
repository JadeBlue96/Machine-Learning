...
def load_custom_set(custom_path,image_size):
  X_test = []
  print("Reading custom image set")
  for fl in glob.glob(os.path.join(custom_path, '*.png')):
      image = cv2.imread(fl)
      image = cv2.resize(image, (image_size, image_size), cv2.INTER_LINEAR)    
      X_test.append(image)
  X_test = np.array(X_test)
...
def read_custom_set(custom_path,image_size):
    class DataSets(object):
        pass
    data_sets=DataSets()
    images=load_custom_set(custom_path,image_size)
    data_sets.test=DataSet_small(images)
    return data_sets
